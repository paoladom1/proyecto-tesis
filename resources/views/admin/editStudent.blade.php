...
@section('content')
    ...
    <div class="container-fluid container-general">
        ...
        <br>
        <form method="POST" action="/editarEstudiante/{{ $estudiante->id }}" class="row g-3">
            @method('PUT')
            @csrf
            <div class="col-md-6">
                    ...
                    <select class="form-control form-select" name="carrera_id">
                        @foreach ($carreras as $carrera)
                            <option value="{{ $carrera->id }}"
                                <?= $estudiante->carrera_id == $carrera->id ? 'selected="selected"' : '' ?>>
                                {{ $carrera->nombre_carrera }}

                            </option>
                        @endforeach
                    </select>
                    <label for="rol">Carrera Universitaria</label>
                </div>
                <div class="form-group form-floating mb-3 mt-3">
                    ...
                    <label for="rol">Grupo de Trabajo </label>
                </div>
            </div>
